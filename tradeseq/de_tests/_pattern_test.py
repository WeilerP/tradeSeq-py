...
class PatternTest(BetweenLineageTest):
    ...
    def __call__(
        self,
        start_times: Union[np.ndarray, float, None] = None,
        end_times: Union[np.ndarray, float, None] = None,
        n_points: int = 6,
        ...
        Corresponds to the PatternTest in tradeseq for start_times and end_times None.
        Corresponds to the earlyDeTest in tradeseq if start_time and end_time are given as a float (in tradeseq of a knot location)

        Parameters
        ----------
        start_times
            Pseudo time starting from which lineage should be compared to other lineages. Either a np.ndarray of shape (``_lineages``,)
            specifying individual start times for all lineages or a float specifying the same start time for all lineages.
        end_times
        ...
        n_points
            ...
            Note that if ``end_time - start_time`` is not the same for every lineage, the lineage length is effectively normalized.
        pairwise_test
